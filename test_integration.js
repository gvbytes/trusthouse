...
async function runTests() {
  ...
  // 2. Cryptographic Encryption/Decryption Integrity check
  ...
  // 3. OTP session creation, validation & rate limiting
  ...
  // Test OTP request rate limits (max 3 sends per window)
  try {
    await requestOTP(testPhone); // Send 1
    await requestOTP(testPhone); // Send 2
    await requestOTP(testPhone); // Send 3
    await requestOTP(testPhone); // Send 4 (Should throw rate limit error)
  ...
  // 4. Webhook cryptographic HMAC signature verification check
  ...
  // 5. Daily Payout calculations verification
  const payoutDate = '2026-06-25';
  
  // Clear any existing records for test items
  ...
  // Seed household and worker using Prisma
  ...
  // Standard 8 hour wage = 800. Deduct 1.5% fee (12) + 18% GST on fee (2.16) = 14.16 deductions. Final = 785.84
  ...
  // 7. Supabase Storage upload & download signed URL checks
  await initStorage();
  ...
  // 8. Platform Health Dependency Pinger checks
