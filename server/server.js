...
// CORS configured to restrict origins. Fallback to common dev server if FRONTEND_URL is not set.
...
// Specific Auth Rate Limiter (Prevent OTP pumping / brute force)
...
// Public Worker Verification Lookup Endpoint (Rate-limited, minimized PII)
...
// Mount the dashboard UI under authentication check
app.use('/admin/queues', authenticateToken, requireRole(['admin']), serverAdapter.getRouter());

// Register background queue processors
...
// Replacement Engine Queue Processor
...
// ID Card Dispatch Queue Processor
...
// Scheduled Jobs Queue Processor
...
// Setup repeatable cron jobs for the 10 scheduled operations on boot
async function setupScheduledJobs() {
  ...
  try {
    ...
    // Cron jobs configured with IST timezone mapping (using UTC conversion)
    const jobs = [
      { name: 'dailyConfirmation', cron: '30 3 * * *' },  // 9:00 AM IST = 3:30 AM UTC
      { name: 'absenceCheck', cron: '30 4 * * *' },        // 10:00 AM IST = 4:30 AM UTC
      { name: 'absenceEscalation', cron: '30 5 * * *' },   // 11:00 AM IST = 5:30 AM UTC
      { name: 'dailyPayout', cron: '30 12 * * *' },        // 6:00 PM IST = 12:30 PM UTC
      { name: 'ratingRequest', cron: '30 14 * * *' },      // 8:00 PM IST = 2:30 PM UTC
      { name: 'loyaltyCheck', cron: '30 2 * * 1' },        // Mondays 8:00 AM IST = 2:30 AM UTC
      { name: 'cardExpiryCheck', cron: '30 3 1 * *' },     // 1st of month 9:00 AM IST = 3:30 AM UTC
      { name: 'metricsSnapshot', cron: '29 18 * * *' },    // 11:59 PM IST = 6:29 PM UTC
      { name: 'weeklyReport', cron: '30 14 * * 0' },       // Sundays 8:00 PM IST = 2:30 PM UTC
      { name: 'lowRatingAlert', cron: '30 15 * * *' }      // 9:00 PM IST = 3:30 PM UTC
    ];
...
app.post('/api/auth/otp/request', validate(otpRequestSchema), async (req, res) => {
  ...
  try {
    ...
    // Send OTP via notification agent
...
app.get('/api/workers/profile', authenticateToken, requireRole(['worker']), async (req, res) => {
  try {
    ...
    // Decrypt Aadhaar if stored to show details securely
    let rawAadhaar = null;
...
app.post('/api/workers/kyc/initiate', authenticateToken, requireRole(['worker']), validate(kycInitiateSchema), async (req, res) => {
  ...
  try {
    ...
    // Call KYC agent to generate Persona Inquiry
...
// Worker Daily Attendance Check-In / Check-Out
app.post('/api/workers/attendance', authenticateToken, requireRole(['worker']), validate(attendanceSchema), async (req, res) => {
  ...
  try {
    ...
    if (action === 'checkin') {
      ...
      // Trigger replacement agent via Bull.js background queue
...
// Toggle worker availability (on-call status)
...
// Generate / retrieve signed URL for worker ID card
app.get('/api/workers/idcard', authenticateToken, requireRole(['worker']), async (req, res) => {
  try {
    ...
    // Generate or retrieve signed PDF URL
...
// Get worker active & historical assignments
...
// Get worker earned benefits (based on trustScore)
app.get('/api/workers/benefits', authenticateToken, requireRole(['worker']), async (req, res) => {
  try {
    ...
    // Dynamic platform fee discount based on worker's trust score
...
/* =========================================================================
   API ROUTES - HOUSEHOLDS
   ========================================================================= */

// Get authenticated household profile
...
// Find nearby verified workers for hire
app.get('/api/households/workers/nearby', authenticateToken, requireRole(['household']), async (req, res) => {
  try {
    ...
    // 2. Set Redis cache if lookup missed
    if (redis) {
      try {
        await redis.set(cacheKey, JSON.stringify(sorted), 'EX', 300); // 5 minutes expiration
...
// Book a worker (Razorpay Order creation server-side)
app.post('/api/households/bookings/create', authenticateToken, requireRole(['household']), validate(bookingCreateSchema), async (req, res) => {
  ...
  try {
    ...
    // Create Razorpay order (Simulation/Test key fallback)
    const amount = worker.hourlyRate * 8; // standard 8 hours
    const commission = amount * 0.014; // 1.4% platform commission from household
    ...
    // Save active booking (Assignment)
...
// Fetch active bookings for household
...
// Household Subscription Checkout (e.g. upgrade to PLUS tier)
app.post('/api/households/subscription/checkout', authenticateToken, requireRole(['household']), async (req, res) => {
  ...
  try {
    ...
    // Update household plan directly (mocking checkout success)
    await prisma.household.update({
      where: { id: household.id },
      data: { plan: plan === 'PLUS' ? 'PLUS' : 'BASIC' }
    });

    // Create Payment record representing subscription payment
    await prisma.payment.create({
      data: {
...
// Double-sided Rating Feedback Submission
app.post('/api/ratings', authenticateToken, validate(ratingSubmitSchema), async (req, res) => {
  ...
  try {
    ...
    // Recalculate average and adjust trust scores dynamically
...
/* =========================================================================
   PUBLIC WORKER VERIFICATION ENDPOINT
   ========================================================================= */

// Minimized field public lookup to prevent harvesting
app.get('/api/verify/:workerCode', publicVerifyLimiter, async (req, res) => {
  ...
  try {
    ...
    if (redis) {
      try {
        await redis.set(cacheKey, JSON.stringify(responseData), 'EX', 3600); // 1 hour expiration
...
app.post('/api/webhooks/razorpay', async (req, res) => {
  ...
  // Validate Razorpay webhook signature locally using SHA256 HMAC
  ...
  try {
    if (event.event === 'order.paid' || event.event === 'payment.captured') {
      ...
      if (orderId && paymentId) {
        ...
        // Update Payment status to SUCCESS and save paymentId
      ...
      if (!existingPayment) {
        // Find household by subscriptionId (which we stored as razorpayOrderId)
        const paymentRecord = await prisma.payment.findFirst({
...
// Mock SMS/WhatsApp Reply Webhook for candidate response simulation
app.post('/api/webhooks/sms-reply', async (req, res) => {
  const { phone, text } = req.body; // text: 'YES' or 'NO'
  ...
  try {
    ...
    // Find the pending replacement offer for this worker
    ...
    if (answer === 'YES') {
      // Confirmed: update replacement record
      ...
      // Update the assignment to link to the new candidate worker
      ...
      // Notify candidate worker and household of success
      ...
      // Set resolution state in Redis to stop the sequential offering loop
      if (redis) {
        await redis.set(`replacement:response:${activeReplacement.id}`, 'YES', 'EX', 3600);
      }
    } else {
      // Rejected: update replacement record
      await prisma.replacement.update({
        where: { id: activeReplacement.id },
        data: {
          status: 'REJECTED',
...
// Trigger daily payout job manually for testing
...
// Local Mock Storage File Download Handler
...
// Trigger a test Redis/Bull.js job
...
app.get('/api/admin/logs', authenticateToken, requireRole(['admin']), async (req, res) => {
  try {
    ...
    // Map to legacy system_logs shape
...
// Admin Metrics Dashboard Aggregator
...
// Admin Pending KYC Review Queue
...
// Admin Disputes list (Low ratings, unresolved replacements)
...
// Admin Fraud Heuristic Detection Flags
app.get('/api/admin/fraud-alerts', authenticateToken, requireRole(['admin']), async (req, res) => {
  try {
    // Heuristic 1: Duplicate bank account numbers across worker profiles
    ...
    // Heuristic 2: Excessive individual payout amounts (> 5000 INR)
    ...
    // Heuristic 3: Rapid replacement request patterns in the last 24h
...
// Global Error Handler (Prevents stack trace leaks in client responses)
