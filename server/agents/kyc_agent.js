...
/**
 * Creates a Persona Inquiry server-side for the worker.
 * Returns the Inquiry ID and session token (or link).
 * @param {string} workerId Unique ID of the worker
 * @param {string} name Worker's name
 * @returns {object} { inquiryId, sessionUrl }
 */
export async function createPersonaInquiry(workerId, name) {
  ...
  if (!PERSONA_API_KEY || PERSONA_API_KEY.includes('mock') || PERSONA_API_KEY.includes('YOUR_')) {
    ...
    // Save reference in worker table
  ...
  try {
    ...
    // Hosted flow redirect link or SDK session link
...
/**
 * Validates the Persona Webhook HMAC signature.
 * Format: Persona-Signature: t=1612345678,v1=sha256_hash
 */
...
/**
 * Processes Persona Webhook payload.
 * States:
 * - inquiry.approved -> VERIFIED
 * - inquiry.failed / inquiry.declined -> FAILED
 * - inquiry.needs-review -> NEEDS_REVIEW
 */
export async function processPersonaWebhook(eventPayload) {
  const event = eventPayload.data;
  const eventType = eventPayload.data.type; // 'inquiry.approved', etc.
  const inquiryId = event.id;
  const status = event.attributes.status; // 'approved', 'declined', 'needs_review'
  ...
  // Fetch worker info to confirm existence
